...
export async function isDeveloper(user: User): Promise<boolean> {
...
export async function isMFAEnabled(user: User): Promise<boolean> {
  if (
...
export async function getRobloxMemberGroups(user: number): Promise<
  {
...
export async function isUserInGroup(
  ...
  const userGroupData = await getRobloxMemberGroups(user);
...
export async function getRobloxUserFriends(user: number): Promise<number[]> {
  const cachedData = await redis
...
export async function getRobloxAssetOwnership(
  user: number,
  item: number,
  itemType?: string,
): Promise<boolean> {
...
export async function getRobloxPlatformBadges(user: number): Promise<number[]> {
  /*
...
export async function getRobloxUserProfile(user: number): Promise<{
  description: string;
  created: Date;
  isBanned: boolean;
...
export async function getRobloxGroupAffiliates(
  group: number,
...
export async function verify(
  ...
  self: boolean = true,
  interaction?:
    ButtonInteraction | CommandInteraction | UserContextMenuCommandInteraction,
  ...
  if (!verifyApiData?.ok) {
    ...
    if (unvRoles.length) await member.roles.add(unvRoles).catch(console.error);
    return {
      content: self
  ...
  const userProfileData = await getRobloxUserProfile(robloxUserId);
  ...
  const groupData = await getRobloxMemberGroups(robloxUserId);
  ...
  for (const bind of binds) {
    ...
    switch (bind.type) {
      ...
      case "friend":
        ...
        const friends = await getRobloxUserFriends(robloxUserId);
        giveRole = friends.includes(bind.friend);
        break;

      default:
        if (!bind.asset) continue;
        giveRole = await getRobloxAssetOwnership(
          robloxUserId,
          bind.asset,
          bind.type,
        );
...
export async function isPremium(guild: Guild): Promise<boolean> {
  ...
  if (!guild.client.application?.owner) await guild.client.application?.fetch();
