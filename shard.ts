...
bot.on('threadCreate', async function (thread): Promise<void> {
  const settings = await mongo.collection('settings').findOne({ guild: thread.guildId }).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
  ...
  const channel = await thread.guild.channels.fetch(settings.threadCreateLogChannel).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
  if (!channel || channel.type !== 'GUILD_TEXT' || !thread.guild.me?.permissionsIn(channel).has('SEND_MESSAGES')) return
  await channel.send({ embeds: [embed] }).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
})
...
bot.on('threadDelete', async function (thread): Promise<void> {
  ...
  const channel = await thread.guild.channels.fetch(settings.threadDeleteLogChannel).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
  if (!channel || channel.type !== 'GUILD_TEXT' || !thread.guild.me?.permissionsIn(channel).has('SEND_MESSAGES')) return
  await channel.send({ embeds: [embed] }).catch(e => {
...
bot.on('threadUpdate', async function (oldThread, newThread): Promise<void> {
  const settings = await mongo.collection('settings').findOne({ guild: newThread.guildId }).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
  ...
  if (!oldThread.archived && newThread.archived) {
    ...
    newThread.members.cache.each(threadmember => {
      if (!oldThread.members.cache.has(threadmember.id)) actionstring += `<@${threadmember.id}> `
    })
  } else if (oldThread.members.cache.size > newThread.members.cache.size) {
    actionstring += `Members removed from thread <#${newThread.id}>:\n`
    oldThread.members.cache.each(threadmember => {
      if (newThread.members.cache.has(threadmember.id)) actionstring += `<@${threadmember.id}> `
    })
  ...
  if (!channel || channel.type !== 'GUILD_TEXT' || !newThread.guild.me?.permissionsIn(channel).has('SEND_MESSAGES')) return
  await channel.send({ embeds: [embed] }).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
})
...
bot.on('voiceStateUpdate', async function (oldState, newState): Promise<void> {
  ...
  const settings = await mongo.collection('settings').findOne({ guild: newState.guild.id }).catch(e => {
    process.env.DSN ? Sentry.captureException(e) : console.error(e)
  })
  if (!settings) return
  const embed = new MessageEmbed()
  ...
  else if (newState.selfDeaf !== oldState.selfDeaf && settings.voiceDeafenLogChannel) {
  ...
  else if (newState.serverMute !== oldState.serverMute && settings.voiceMuteLogChannel) {
    actionstring += `was ${newState.serverMute ? 'muted' : 'unmuted'} by a server moderator.`
    logChannel = await newState.guild.channels.fetch(settings.voiceMuteLogChannel).catch(e => {
      process.env.DSN ? Sentry.captureException(e) : console.error(e)
    })
  }
  ...
  else if (newState.channel && oldState.channel && settings.voiceSwitchLogChannel && newState.channelId !== oldState.channelId) {
    actionstring += `switched from <#${oldState.channelId}> to <#${newState.channelId}>`
    logChannel = await newState.guild.channels.fetch(settings.voiceSwitchLogChannel).catch(e => {
      process.env.DSN ? Sentry.captureException(e) : console.error(e)
    })
  }
  if (actionstring === `<@${newState.member.id}> ` || logChannel?.type !== 'GUILD_TEXT' || !newState.client.user || !logChannel?.permissionsFor(newState.client.user.id)?.has('SEND_MESSAGES')) return
  embed.setDescription(actionstring)
  await logChannel.send({ embeds: [embed] }).catch(e => {
